...
class DanielFirstMigration extends Migration
{
    /**
     * Run the migrations.
     *
        ...
        Schema::table('users', function (Blueprint $table) {
            Schema::table('users', function (Blueprint $table) {
                ...
                try {
                    ...
                    (
                    CASE
                      WHEN (mp.daily_percent = 0 ) THEN (mp.min_profit)
                      ELSE mp.daily_percent
                    END
                    )
                    * (floor(ump.days_left / mp.accrual_period) + 
                    case 
                        when floor(ump.days_left / mp.accrual_period) != (ump.days_left / mp.accrual_period)
                        then 1
                        else 0
                    end
                    ) + 
